...
def main():
    ...
    p.add_argument("--config", type=Path, default=ROOT / "configs/voice_cast.json")
    p.add_argument("--only")
    args = p.parse_args()
    cast = json.loads(args.config.read_text(encoding="utf-8"))
    defaults = cast["defaults"]
    output_root = ROOT / "outputs/raw/voice_cast_v1"
    output_root.mkdir(parents=True, exist_ok=True)
    engine = VoxCPMEngine()
    rows = []
    total = len(cast["voices"]) * defaults["candidates"]
    done = 0
    for voice in cast["voices"]:
        if args.only and voice["id"] != args.only:
            continue
        for index in range(1, defaults["candidates"] + 1):
            ...
            if complete(wav):
                ...
                meta = engine.design_voice(
                    ...
                    warmup_patches=defaults["warmup_patches"],
            ...
            rows.append(row)
            print(
                f"[{done}/{total}] {status} {voice['id']} candidate_{index:02d} {meta['duration']:.2f}s",
                flush=True,
            )
