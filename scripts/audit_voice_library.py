...
def main():
    ...
    for voice in lib.list():
        for style in voice.get("styles", {}):
            styles += 1
            try:
                entry = lib.resolve_style(voice["id"], style)
                stats = inspect_audio(entry["audio"])
                if not entry["transcript"]:
                    errors.append(f"{voice['id']}/{style}: empty transcript")
                if (
                ...
                if stats["duration"] < 5.0:
                    warnings.append(
                        f"{voice['id']}/{style}: reference is {stats['duration']:.2f}s; use 5-30s, preferably 8-15s"
                    )
                if stats["peak"] > 0.98:
                    warnings.append(
                        f"{voice['id']}/{style}: peak {stats['peak']:.3f} is close to full scale"
                    )
                report.append(
                    {
    ...
    summary = {
        "characters": len(lib.list()),
        "styles": styles,
        "errors": errors,
        "warnings": warnings,
        "entries": report,
    }
    Path("voices/audit_report.json").write_text(
