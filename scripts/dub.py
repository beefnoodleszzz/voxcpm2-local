...
def main():
    ...
    p.add_argument(
        "--mode",
        choices=["design", "controllable", "ultimate"],
    ...
    p.add_argument(
        "--instruct", default="自然生活化对白，语气克制，停连随语义变化，不要播音腔，不要机械匀速"
    )
    p.add_argument("--candidates", type=int, default=3)
    p.add_argument("--profile", choices=["production", "fast", "balanced", "quality", "ultimate"])
    p.add_argument(
        "--normalization-profile", choices=["none", "dialogue", "narration"], default="none"
    )
    ...
    engine = VoxCPMEngine()
    if args.mode == "design":
        metas = engine.generate_candidates("design_voice", folder, instruct=args.instruct, **common)
    elif args.mode == "controllable":
        ...
        metas = engine.generate_candidates(
            "ultimate_clone", folder, character_id=args.character, style=args.style, **common
        )
